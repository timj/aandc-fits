...
% We believe, firstly, that the question of the number of existing FITS
% versions is at least more subtle than it might appear, and that this
% subtlety has costs to implementers; and secondly that it may now be
% time to revisit the standard's requirements of backward compatibility.
...
\citep{1997ASPC..125..257W}, which discusses this general point in
...
Even if the ``once FITS, forever FITS'' doctrine is taken to require
only perfect backward and forward compatibility, the latter requires
only that a FITS reader will not produce \emph{errors} when confronted
with a `newer' FITS file, but does not require that it will be able to
faithfully interpret and make available all of the content of that
file, nor even that the reader will be reliably able to detect and
report the existence of features it does not recognize.  However much
of a problem this is in practice -- and the backward compatibility
means that it always \emph{should} be feasible to use the most recent
FITS reader -- it creates uncertainty and causes implementers to rely
more on `duck-typing' and heuristics which are
ultimately error prone because it requires the implementer of the parser
to perfectly interpret the signature of any particular set of features
present in the given FITS instance from among other possible features which are
absent. Furthermore, as the format evolves beyond the date of its creation,
the software cannot know how that signature may change and may incorrectly
...
increase. In consequence, it may be time to consider whether the
