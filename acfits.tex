...
disambiguate or associate declared properties in the file with other
information external to the file. These are common problems to solve;
even the 'simple' metadata in figure \ref{fig:fitshead} illustrates this.


In the example FITS header the EPOCH keyword is a source of ambiguity.
The comment indicates it is the epoch of the Modified Julian Date, however,
in other files, EPOCH may be used to declare the epoch for a different
time system (such as ICRS or FK5), and thus, not directly comparable to
the information in the example.


The use of the EXPTIME keyword presents an example of association.
This keyword declares the exposure time, a common property in astronomical
observations. Other FITS files may instead utilize a different keyword for the
exact same meaning, for example, the keyword EXP\_TIME. At the NOAO archive
{\color{red} (NOAO Archive reference here)} there are examples of both
declarations of exposure time. One keyword is valid for some instruments, 
and the other valid for other instruments {\color{red} (NOAO keyword 
dictionary reference here)}.
