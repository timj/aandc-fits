...
(e.g.~HDF5 \citep{2011ASPC..442...53A} and NDF
\citep{2014Jenness}). There are other popular file formats amongst the
...
applications to interoperate \citep{2014Jenness}.  HDX was developed
as a way of combining FITS and NDF into a single unified data model in
XML. HDF5 was chosen to accommodate LOFAR's
...
the files. Counter to the maxim ``once FITS, forever FITS'', FITS readers
are not able to appropriately handle just any version of FITS.
Certainly, were someone to utilize code developed in the early 1980s
to read a FITS file containing a binary table, or a FITS file with
extensions, it would fail because it would have no idea how to handle these
data structures. Suitability for archiving not only means that readers are
backward compatible, but that older readers can gracefully handle newer
...
adopt the meaning ({\color{red} need a reference, used Wikipedia but
some didnt like utilizing that source, other reference?}
...
in the file may be validated and understood.  Questions such as
``are all of the required metadata/data structures present in the file?''
(e.g.~all of the needed keywords occur in the correct places in the file)
and ``are there any non-normative values in the file?'' (all
metadata/data values are within expected bounds) are both questions
answered by ``validation'' (conformance of information in the file to one
or more declared data models).
The fundamental question of ``what are these data you gave me about?''
(e.g.~lists of stars, tables of galaxies, images of dust clouds, etc) may
also be answered when we allow information in the file to be associated
with one or more data models.
It is important to note that these are all questions critical to consumers
of the file.
% and by allowing the file to declare its association to one or
%more data models, we may answer all of these questions without having
%a human inspect the file.
...
disambiguate or associate declared properties between models.
These are common problems to solve and even the `simple' metadata in
Fig.~\ref{fig:fitshead} illustrates this.  Consider the TEMP keyword in
the example. What is this the temperature of? TEMP is a likely keyword
string to appear in other files, how do we know if the TEMP in the other
files is the same one we see in the example?


The use of the EXPTIME
keyword presents a another example of the need for namespaces.
In this case, we know clearly it is the exposure time of the detector
which is a common property in astronomical observations however there may
exist other files which contain this same property, but instead name it
EXP\_TIME\footnote{This is a real example. At NOAO we have some instrument
data models which use one term and vice versa.}.
How can the machine know
that these are the same properties? A machine readable mapping between
the models could be set up. This mapping should be written in a common
shared format (for FITS files) and all downstream software programs would know
that model1:EXPTIME property is the same as model2:EXP\_TIME.


It is clear then that there is a need for namespaced data models,
...
particular namespaced data models.
...
general case. The writing of generalized software programs to detect any data
...
is likely hopeless to determine which data models are present and
map information to appropriate meaning.
...
inspection or specifically written software programs (similar, perhaps, in the
...
complicating the code, and introducing inconsistencies between
...
{\color{red} (Can the preceding sentence and the following be formulated
without reference to a ``new format''? The aim of this paper is to
highlight problems with FITS, not to formulate requirements for a format
to come.)}
...
line in the header using the \texttt{CONTINUE} convention
\citep{2007Continue}.
...
\citep{2006ASPC..351..112B}, used for polynomial distortion coefficients, to the more
