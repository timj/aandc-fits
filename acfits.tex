...
\title{Understanding the capabilities of an astronomical data format:
  Learning from FITS}
...
The FITS standard  \citep{1981A&AS...44..363W} requires that the data are
stored in order of decreasing significance (big endian format).
Sometimes the byte order needs to be checked and swapped to the native
byte order in sytems that use the little endian format when non-native
data formats are not supported (as in implementations of FITS readers
which utilize C, Cython, and so on). Programmers on little endian
