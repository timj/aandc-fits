...
its application to modern astronomical research. We hope to show that 
technologies and research techniques in astronomy have evolved and FITS has 
...
It is our intended goal
in this paper to highlight some selected, important, problems which exist
in the FITS core standard today. 
...
We have grouped the discussion of issues as information interchange
(section~\ref{section_poor_exchange}),
...
(section~\ref{section_poor_large_data_support}).
We address each of these topics in turn below then try to provide an 
analysis of any deeper causes or ``lessons learned'' 
...
\subsection{Lack of serialization/format versioning}
...
\subsection{Inadequate declaration and validation of content meaning}
...
WCS, and these are generally regarded as another strong point as few other data 
formats supply much in the way data models in their standard. The number and 
richness of core data models varies with each format ranging from HDF5 which
does not supply any data models to NDF, itself a data model implemented in HDS, 
which has arguably the richest model in its core definition.
It is a matter of opinion as to whether more data models in the format standard 
are better or not. NDF has its advocates but clearly it is not as well adopted as 
FITS which has a less rich assortment of data models in its standard.
Nevertheless, we believe FITS would benefit from an expansion in its roster of 
standard data models.  We detail some important missing data models below.
...
\subsection{Constrained metadata representation}
...
\subsection{Insufficient support for missing values}
...
\subsection{Awkwardness in representing the associations present in both data and metadata}
...
\subsection{No support for declaring byte order}
...
\subsection{No support for alternative encodings}
...
\subsection{No support for parallel write/read operations}
...
\subsection{No support for streaming imaging data}
...
\subsection{No support for capturing indeterminately sized datasets via streaming}
...
\subsection{No support for declaration of virtual and distributed components}
