...
{\color{red} \textit{Content doesnt seem to fit in flow here}
...
\caption{Example header of a representation of an AST WCS object in a
  FITS header  when the mapping is too complex to be represented using the FITS-WCS
  standard.}
\label{fig:asthead}
\end{minipage}
\end{figure*}
...
It is critical that this machinery be integral to the format.
...
The problems which we have described are real and significant,
and we have tried to draw conclusions about their origin.

We do not wish to recommend a particular solution here to any 
particular problem.
Instead we hope that action to correct deficiencies should flow from 
constructive community discussion and offered solutions to these problems. 
...
instead, and the format serialization flexible enough to handle
...
This in fact, is being done now, and two other papers in this series will
provide analysis of JPEG2000 \citep{2014arXiv1401.7433P}
and NDF formats \citep{2014Jenness} \ldots
...
\item Data models should be able to reference to (associate with) 
other data models via standard within the format 

\end{itemize}

Finally, we feel consideration should be given to creation of a common API
