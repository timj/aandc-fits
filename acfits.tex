...
fundemental question ``what does this data you gave me represent?'' (e.g.~lists 
...
the example. Is this a temperature or does in imply some type of temporary
file or resource? If it is a temperature then what is this the temperature 
of? TEMP is a likely keyword string to appear in other files, how do we 
know if the TEMP in the other files is the same one we see in the example?
...
To make matters worse, does EXP\_TIME mean the start of the exposure, or 
does it mark its mid-point?. What about EXPTIME?
As you can see, its non-trivial matter for the machine to deterime whether 
these are the same properties. 


Namespacing provides a common means to reconcile these issues. Essentially,
namespacing means tagging the data model and its elements with a unique 
identifier.  A machine readable mapping may then be set up so that downstream 
software programs would know that model1:EXPTIME property is the same as 
model2:EXP\_TIME.
