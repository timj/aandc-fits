...
\subsection{Lesson 3. The serialization should not limit expression of desired data models}

{\color{red} \textit{Need to verify this statement:} FITS was originally designed around capture and transport of a single
data model, that being the two dimensional image.} In this regard FITS has 
performed reasonably well, however, as we have seen, over the intervening 
years, more and more demands on the type of data (models) it may hold
have been made, and all of this having been done without adjustment
to formatting limitations as have been enumerated in 
section~\ref{subsection_information_representation}.


Holding the line on format serialization changes via the 
``Once FITS, Always FITS'' maxim has been harmful. Specifically, we mean 
the lack of support within the format for basic data structures beyond 
scalar values and the severe formatting restrictions on FITS cards. As 
we have shown, these limitations result in significant problems in 
realizing existing data models, such as the WCS, and make it difficult 
to create new common models of high value, such as for scientific errors, 
units and provenance/history. 
...
As we have already noted, in many cases it is possible to create one or 
more conventions to expand the expression of the metadata but doing so will
...
We feel that the format should simply "get out of the way" of the 
data model. In practice this means having very few hard-coded rules 
within the format itself such as the 2880 byte record. Schema should 
be capable of describing \textit{data and metadata} blocks instead, and the 
format serialization flexible enough to handle most data models.
A successful data format design will require an through analysis of 
data model requirements, but we can anticipate a minimum of desired 
changes which include:

\begin{itemize}

\item Specification of data model in machine readable document (e.g. schema)

\item Keyword names and values must be allowed to be longer

\item An expanded character set (for both metadata and data) 

\item The format should be able to be express, as part of its standard, 
many basic structures in metadata. These include things such as non-scalar 
values, sets, lists, and objects.

\item Data modeling should be able to specify the capture ``data'' too. This
certainly means specifying byte order, but should go beyond that so that a
data model might encapsulate information in more advanced structures than FITS
current basic 2D image, data cube and table models. 

\item Data modeling should allow use and extension of other data models
...
As we have illustrated in section~\ref{section_poor_large_data_support} there 
are a number of new use cases for the storage and distribution (transport) 
of FITS data. These use cases, which range from supporting large data access, 
transport and storage to representation of information held in ``non-file''  
instances (such as a database) are unachievable in FITS currently. The root
reason is that for data modeling in FITS, as pointed out previously, it is 
impossible to model new data structures (as opposed to models which just need
metadata modeling).
...
If the capability to model the layout and lower-level access of the data 
held within the format existed, then it would devolve to a straightforward 
exercise to specify this information so as to optimize for desired transport
and storage use cases.  

Furthermore, consideration should be given to creation of a common API
...
Consider that XML, for example, as at least 2 standard API for access its 
...
or SAX). There should be an overarching data model for all API, which 
should allow implementors to declare the features it supports, such as 
file, streaming parallel IO and distributed files.
...
provide analysis of JPEG2000 and NDF formats \citep{2014Jenness} \ldots.
}
