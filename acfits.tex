...
% I suggest here adding 
% RPFITS - ATNF data-recording format \footnote{http://www.atnf.csiro.au/computing/software/rpfits.html}
% and 
% CASA MeasurementSet \footnote{http://casa.nrao.edu/Memos/229.html}
...
that co-opts commentary keywords to realize the hierarchy. The other 
...
commentary keywords for non-commentary purposes. As another example, 
...
unlike a hard-drive is a serial access device.  The concept
of the serial streaming effectively defined the FITS data
model.  Although the tapes are still well used for archiving purposes,
such a mode is no longer practical, and the files are typically 
transferred to a hard-drive before being read. However, such
a serial nature of FITS has become a significant limitation for the efficient 
work with the large multidimensional datasets.

Several new instruments, especially in radio astronomy (ASKAP~\citep{ASKAP..09},
MWA~\citep{2012rsri.confE..36T}, LOFAR~\citep{2013A&A...556A...2V}, and SKA~\citep{ska-exascale}) 
have been already producing or planning to produce
in the near future the spectral-imaging data-cubes of unprecedented volumes in the order of
tens and hundreds of petabytes per year. Due to the increased spacial and frequency 
resolutions the individual data sets can be now be expected to be as large as tens of
terabytes. 

Servicing such data as images in FITS is going to encounter
significant performance fallbacks due to inability of FITS to provide
acceptable performance for data-cubes larger than just a few gigabytes.

\subsubsection{No support for maintaining the integrity of distributed data sets}

One possible solution for the large datasets is to generate many small related subsets. 
However, FITS does not have any mechanism to maintain the
integrity of such distributed datasets on its own. Any support for the integrity
of such datasets would have to require an external to FITS proprietary solution that 
potentially might limit the interoperability and portability of the data.
...
Servicing the large data sets to the end user also requires significant
rethinking of the framework in general. FITS can only reasonably support the
cutout framework, that is when a sub-cube is extracted from a dataset.
FITS internal serialisation is not optimal for random read operations. 
The random reads required to produce a cutout cause a high latency 
due to the excessive \emph{seek} operation when going from one 
frame of a cube to another. Although, this problem will have less impact
on the performance once the solid state discs (SSD) replace HDD technology,
the cutout framework is generally ineffective for visual exploration or
inspection of multi-terabyte imaging datasets.

The solution could lie in adopting a comprehensive approach similar to the one
employed in JPEG2000 \citep{2003SPIE.5150..791T}. The adopted
serialisation and encoding of imaging data  in JPEG2000 supports multiple 
representations (e.g., multiple resolutions or fidelities) of the image in 
a single master file, but which can be distributed across the network. Encoding
of the image can be transparently to the end user optimised to the used of the image.
The used encoding schema also enables streaming the data
progressively to the end-user, displaying an image as soon
as the first data become available. \citealt{2014arXiv1403.2801K} and 
...
\subsubsection{Limitted support for compression}

The large data volumes are directly translated into large cost of the archive and 
network infrastructures, and the high latency of data retrieval. An effective 
compression is going to be a must for the large datasets.

The \texttt{cfitsio} library supports several types of binary and one type of 
image compression by convention \citep[see
e.g.,][\href{http://ascl.net/1010.002}{ascl:1010.002}]{2000ASPC..216..551P, 
2007ASPC..376..483S,2009PASP..121..414P}. As FITS on its own does 
not provide a mechanism to store compressed data or images, the binary 
table extensions have been used to store the encoded data in both cases,
however, to access an image, image \texttt{cfitsio} API must be used. 

Is this sufficient?

Although, FITS acknowledges the existence of convention for compression,
it does not specify the encoding or compression of the data on its own. The 
serialisation of FITS is not optimal in order to support the advanced 
compression techniques for multidimensional multi-terabyte datasets as the one 
described in the previous section. \citealt{2014arXiv1403.2801K} demonstrate 
...
Another problem, of course, is that the compatibility for the \texttt{cfitsio} 
...
of issues which include the ability to declare byte order, to make associations
between data and metadata, and to create data models which extend both
...
companion papers \citep{2014arXiv1403.2801K,2014Jenness}), and offered solutions to these problems.
