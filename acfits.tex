...
\citep{P91_adassxxiii}). There are other popular file formats amongst the
...
appears in figure \ref{fig:fitshead}. 
...
in figure \ref{fig:fitshead}, the only keyword which implies any type of 
...
is however, false. Accounting may differ, but there have been at least 3 
...
were not 2 earlier recognized versions? It is clear then that the format 
...
software are not able to appropriately handle just any version of FITS. 
...
data structures. Suitability for archiving not only means that software are
backward compatible, but that older software can gracefully handle newer 
...
(ex. all of the needed keywords occur in the correct places in the file)
...
(ex. lists of stars, tables of galaxies, images of dust clouds, etc) may
...
figure \ref{fig:fitshead} illustrates this.  Consider the TEMP keyword in
...
shared format (for FITS files) and all downstream software would know
...
to inspect the file, or to write special purpose software targeted to 
